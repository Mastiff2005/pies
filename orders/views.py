        ...
        if form.is_valid():
            ...
            if not request.user.is_staff:
	            send_mail(f'Новый заказ №{order.id} от {request.user}!',
	                      'Новый заказ на сайте CAKE POINT. Перейдите в панель управления для уточнения информации.', 
	                      'info@cake-point.ru', ['info@cake-point.ru', 'pavel402@bk.ru'],
	                      fail_silently=False)
	            send_mail(f'Ваш заказ №{order.id} на CAKE-POINT.ru', 'Благодарим за Ваш заказ! С Вами свяжется менеджер для уточнения деталей заказа.',
...
def admin_order_detail(request, order_id):
    ...
    user = order.user
    return render(request, 'admin/orders/order/detail.html', {'order': order, 'user': user})
...
def admin_order_pdf(request, order_id):
    ...
    response['Content-Disposition'] = 'filename="Nakladnaya_{}.pdf"'.format(invoice_num)
...
def get_selection(request):  # получить выгрузку всех позиций заказа
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return_goods = OrderItem.objects.filter(      # начало модификации
            ...
            for item in selection:
                ...
                selection_modified.append({'product': selected_product,
                                           ...
                                           'price': price,
                                           'quantity': quantity})
            return render(request, 'orders/order/selection.html', {'selection_modified': selection_modified, 
            'form': form, 'return_goods': return_goods_modified})
        return render(request, 'orders/order/selection.html', {'selection_modified': selection_modified, 
        'form': form})
    form = SelectionForm()
    return render(request, 'orders/order/selection.html', {'selection_modified': selection_modified, 'form': form})
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            for user in users:
                ...
                user_orders_returns_sum = abs(round(user_orders_returns_sum, 0))
                user_orders_sum = round((user_orders_sum + user_orders_returns_sum), 0)
                user_margin = round((user_orders_sum - user_orders_purc - user_orders_returns_sum), 0)
                if user_orders_sum != 0:
                	user_margin_percent = round((user_margin / user_orders_sum * 100), 0)
                else:
                	user_margin_percent = 0
                if user_orders_count != 0:
                	average = round((user_orders_sum / user_orders_count), 0)
                	user_margin_order = round((user_margin / user_orders_count), 0)
                else:
                	average = 0
                	user_margin_order = 0
                users_orders_sum.append({'user': user,
